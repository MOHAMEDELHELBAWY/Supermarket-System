...
while True:
    ...
    print('1. View items\n2. Add items for sale\n3. Purchase items\n4. Search items\n5. Edit items\n6. View Most Expensive/Least Expensive\n7. Exit')
    ...
    if choice == '1':
        ...
        print('------------------View Most Expensive/Least Expensive------------------')
        
        if items:  
            def get_price(item):
                return item['price']
            
            most_expensive = max(items, key=get_price)
            print('Most Expensive Item:')
            for key, value in most_expensive.items():
                print(f"{key}: {value}")
            
            
            least_expensive = min(items, key=get_price)
            print('Least Expensive Item:')
            for key, value in least_expensive.items():
                print(f"{key}: {value}")
        else:
            print('No items available in the inventory.')
